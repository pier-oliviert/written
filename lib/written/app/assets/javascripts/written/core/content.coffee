class @Written
  constructor: (el) ->
    ...
    event = new CustomEvent('Written:loaded', {bubbles: true})
  ...
  outdated: (observedMutations) =>
    @observer.hold =>
      mutations = new Written.Mutations(observedMutations, @element())
      if mutations.target?
        @updated(mutations)
      ...
      if @element().children.length == 0
        ...
        cursor = new Written.Cursor(0)
        cursor.update(@walker(line), 0)


    event = new CustomEvent('Written:updated', {bubbles: true})
    @element().dispatchEvent(event)
  ...
  linefeed: (e) =>
    ...
    lineFeed = new Written.LineFeed(@cloneNodesFrom(node))
    lineFeed.injectAt(offset)

    cursor = new Written.Cursor(offset + 1)
